...
include 'koneksi.php';
$npm = $_GET['npm'];
$data = mysqli_query($koneksi, "SELECT * FROM t_mahasiswa WHERE npm = $npm");
...
<div class="bg-white p-8 rounded-2xl shadow-xl w-full max-w-xl">
    ...
    <form action="proses_editmahasiswa.php" method="POST" class="space-y-4">
        ...
        <input type="text" name="namaMHS" value="<?= $row['namaMHS'] ?>" class="w-full px-4 py-2 border rounded-lg">
        <input type="text" name="prodi" value="<?= $row['prodi'] ?>" class="w-full px-4 py-2 border rounded-lg">
        <input type="text" name="alamat" value="<?= $row['alamat'] ?>" class="w-full px-4 py-2 border rounded-lg">
        <input type="text" name="noHP" value="<?= $row['noHP'] ?>" class="w-full px-4 py-2 border rounded-lg">
